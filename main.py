...
if refresh_token:
    # On Streamlit Cloud: inject stored refresh token, skip browser OAuth
    ...
    # Local: force re-login every run
...
def top_artists():
    topArtists=sp.current_user_top_artists(limit=10)
...
def followed_artists():
    followedArtists=sp.current_user_followed_artists(limit=10)
    return followedArtists['artists']['items']
def playlists():
    playlists=sp.current_user_playlists(limit=10)
    return playlists['items']
def saved_tracks():
    savedTracks=sp.current_user_saved_tracks(limit=10)
